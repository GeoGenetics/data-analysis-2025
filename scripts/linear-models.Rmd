...
#if (!require("performance")) devtools::install_github("easystats/performance") # Library for evaluating performance of linear model
#if (!require("see")) devtools::install_github("easystats/see")
...
This module serves as an introduction to linear models

We will cover the following topics

* Fitting a simple linear model
* Interpretting a fitted linear model
...
We'll begin by focusing on the relationship between two of the continuous variables: body size (in kg) and total amount of sleep (in hours). Plot TotalSleep as a function of BodyWt to see what this relationship looks like:
...
myfirstmodel <- lm(TotalSleep ~ log(BodyWt), data=allisontab) 
ggplot(allisontab) + 
  geom_point(aes(x=log(BodyWt),y=TotalSleep)) +
  geom_abline(intercept=myfirstmodel$coefficients[1],slope=myfirstmodel$coefficients[2])
```
...
Try fitting a linear model for $\bf{TotalSleep}$ as a function of brain weight ($\bf{BrainWt}$). Keep in mind that this is a size measurement that might span multiple orders of magnitude, just like body weight. What are the estimated slope and intercept coefficients? Which coefficients are significantly different from zero? What is the proportion of explained variance? How does this compare to our previous model including $\bf{BodyWt}$?

```{r interpret1, exercise = TRUE}

```
```{r interpret1-solution}
newmodel <- lm(TotalSleep ~ log(BrainWt), data=allisontab) 
summary(newmodel)
```


Now, plot the linear regression line of the above exercise on top of your data.

```{r interpret2, exercise = TRUE}

```
```{r interpret2-solution}
newmodel <- lm(TotalSleep ~ log(BrainWt), data=allisontab) 
ggplot(allisontab) + 
  geom_point(aes(x=log(BrainWt),y=TotalSleep)) +
  geom_abline(intercept=newmodel$coefficients[1],slope=newmodel$coefficients[2])
```
...
q) Plot the output of a simulated linear model with 40 data points, an intercept of 1.5 and a slope of 3. Simulate from the same model one more time, and plot the output of the two simulations.

```{r gen1, exercise = TRUE}

```
```{r gen1-solution}
sim_A <- linearmodsim(n=40, beta_0=1.5, beta_1=3)
sim_A2 <- linearmodsim(n=40, beta_0=1.5, beta_1=3)
plot(sim_A[,1],sim_A[,2])
plot(sim_A2[,1],sim_A2[,2])
```

b) Now, generate a new simulated data and fit it using the `lm()` function. Does your fit match your simulations?

```{r gen2, exercise = TRUE}

```
```{r gen2-solution}
sim_B <- linearmodsim(n=40, beta_0=1.5, beta_1=3)
fitB <- lm(response ~ explan, data=sim_B)
summary(fitB)
```

c) Try increasing the sample size (say, to 10,000 data points), and repeat the `lm()` fitting. How does this influence the accuracy of your fitted model? Try simulating and fitting multiple times to get an idea of how well you can estimate the parameters.

```{r gen3, exercise = TRUE}

```
```{r gen3-solution}
sim_C <- linearmodsim(n=10000, beta_0=1.5, beta_1=3)
fitC <- lm(response ~ explan, data=sim_C)
summary(fitC)
```

d) Try changing the standard deviation of the simulated residual errors (make `sigma.res` smaller or larger), and repeat the `lm()` fitting. How does this influence the accuracy of your fitted model? Why do you think this is?

```{r gen4, exercise = TRUE}

```
```{r gen4-solution}
sim_D <- linearmodsim(n=10000, beta_0=1.5, beta_1=3, sigma.res=100)
fitD <- lm(response ~ explan, data=sim_D)
summary(fitD)
sim_E <- linearmodsim(n=10000, beta_0=1.5, beta_1=3, sigma.res=0.01)
fitE <- lm(response ~ explan, data=sim_E)
summary(fitE)
```

## A multivariate linear regression

We can also make our model a bit more complex. Let's say we want to incorporate another variable - life span (in years) - into the model predicting sleep:

```{r}
multimodelA <- lm(TotalSleep ~ 1 + log(BodyWt) + LifeSpan, data=allisontab)
summary(multimodelA)
```
...
```{r}
multimodelB <- lm(TotalSleep ~ 1 + log(BodyWt) + log(Gestation), data=allisontab)
summary(multimodelB)
```
...
What happens if we use all three variables ($\bf{LifeSpan}$, $\bf{log(BodyWt)}$ and $\bf{log(Gestation)}$) as explanatory variables? Do we get a much better model fit than we using only two? What if we use $\bf{log(Gestation)}$ as the only explanatory variable?

```{r multi1, exercise = TRUE}

```
```{r multi1-solution}
multimodelC <- lm(TotalSleep ~ 1 + log(BodyWt) + log(Gestation) + LifeSpan, data=allisontab)
summary(multimodelC)
multimodelD <- lm(TotalSleep ~ 1 + log(Gestation), data=allisontab)
summary(multimodelD)
```

Keep in mind that we generally want a good fit with as simple a model as possible (Occam's Razor). There will always generally be a trade-off between how much variance the model can explain and how complex the model is (how many parameters it has). In the course on Advanced Topics in Data Analysis, we'll find ways to compare models that are more complex to those that are more simple.
