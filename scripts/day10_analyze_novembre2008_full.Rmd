...
where $\theta$ represents the rotation angle (in unit of radians) counterclockwise from the positive x axis.
...
First we'll need to read in the PCA results and color map
...
We can obtain the optimal rotation angle in degrees from the rotation matrix returned in the `rotation` element of the results object. A rotation matrix defined as
...
will rotate points counterclockwise by the angle $\theta$ (in unit of radians) from the positive x axis. Using the first element of the matrix we can therefore obtain the angle in degrees as

```{r procrustes5}
theta1 <- atan2(rot$rotation[2,1], rot$rotation[1,1]) * 180 / pi
```
Our best-fitting rotation angle is `r round(theta1, 0)` degrees.

<br />

### Plotting the rotated PCA
...
<br />
Finally, we can use our previously obtained rotation angle `theta1` of `r round(theta1, 0)` degrees to adjust the angles for the axis label text. 
...
In the second part of the exercise, we will follow the paper and build a predictor for an individual's geographic location from its PCA position, using linear regression. As test case, we take all individuals from Austria, remove them from the dataset and use the remaining individuals to build the model. Then, we use the built model to predict the location of the test individuals. 
...
<br />
To evaluate the accuracy of the predicted locations, we can calculate the average distance between the predicted and true location for each sample using the "Haversine" great circle distance, implemented in the `distHaversine()` function of the `geosphere` R package.

```{r pred4}
pred_accuracy <- d1 %>%
  rowwise() %>%
  mutate(dh = distHaversine(c(longitude_pr, latitude_pr), c(longitude, latitude))) %>%
  ungroup() %>%
  summarise(dh_avg_km = mean(dh)/1e3)
```

The average distance of the predicted location for the individuals from Austria is `r round(pred_accuracy$dh_avg_km, 0)` km.

<br />
Finally, we can plot the inferred versus actual sample locations using the basic map plotting functionality in `ggplot2`. We reshape the results table into a longer format and plot the predicted and original locations on a map of Europe
